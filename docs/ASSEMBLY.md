...
Somebody not at work is a day they give their orders nothing. Planned leave and OnShift come from `ASSY_Operator`; **Not in today** on the operator's own popup is the supervisor's record of a phone call, and it is dated so a known absence can go in ahead of the day; dragging the name into the **Absent** roll in the crew column writes the same thing in one gesture, and dragging it back into **Free** says they turned up after all. The schedule then runs the order on the hands that are there — the bar lengthens with one fewer, and shows a gap when the last one goes — and the load histogram, the crew suggestion and the pickers all stop offering that person. Nobody is taken off an order for being away: a half-built order belongs to whoever was building it, so the chip is marked instead, and the **Absent** roll in the crew column names who is out and counts the begun orders nobody else is on.
